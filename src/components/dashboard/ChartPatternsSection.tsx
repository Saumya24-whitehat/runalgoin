...
export function ChartPatternsSection() {
  ...
  const getCurrentPatterns = (): PatternItem[] => {
    ...
    if (mainTab === "stocks") {
      ...
      return stocksSubTab === "candlesticks" 
        ? (stocksData.candlesticks || [])
        : (stocksData.chartPatterns || []);
    } else {
      const fnoData = data.fno;
      if (!fnoData) return [];
      switch (fnoSubTab) {
        case "candlesticks": return fnoData.candlesticks || [];
        case "chartPatterns": return fnoData.chartPatterns || [];
        case "priceAction": return fnoData.priceAction || [];
        default: return [];
      }
    }
  };
            ...
            <div className="divide-y divide-border">
              {filteredPatterns.slice(0, 50).map((pattern) => (
                <div key={pattern.id} className="flex items-center gap-3 p-3 hover:bg-muted/50 transition-colors">
                  {/* Pattern Icon */}
                  <div className="w-10 h-10 flex-shrink-0 rounded" style={{ backgroundColor: '#2a2e39' }}>
                    <img 
