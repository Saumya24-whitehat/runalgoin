...
import { createChart, IChartApi, ISeriesApi, ColorType, LineSeries, HistogramSeries, LineStyle } from "lightweight-charts";
import { StrikeData } from "@/types/optionChain";

interface HeatMapOIChartProps {
  data: StrikeData[];
  atm: number;
}

export function HeatMapOIChart({ data, atm }: HeatMapOIChartProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const chartRef = useRef<IChartApi | null>(null);
  const ceSeriesRef = useRef<ISeriesApi<"Histogram"> | null>(null);
  const peSeriesRef = useRef<ISeriesApi<"Histogram"> | null>(null);
  ...
  useEffect(() => {
    ...
    // Create CE OI histogram series
    const ceSeries = chart.addSeries(HistogramSeries, {
      color: '#22c55e',
      priceFormat: {
        type: 'custom',
        formatter: (price: number) => formatYAxis(price),
      },
      priceScaleId: 'right',
    });
    ...
    <div className="h-[350px] w-full">
      <div className="flex items-center justify-center gap-6 mb-2 text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-foreground">CE OI</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded bg-red-500" />
          <span className="text-foreground">PE OI</span>
        </div>
        <div className="flex items-center gap-2">
