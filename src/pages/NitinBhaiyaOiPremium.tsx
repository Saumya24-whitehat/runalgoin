  ...
  return <PageLayout showFooter={false}>
    ...
    <section className="mx-auto w-full max-w-[1500px] py-3">
      <div className="flex items-start justify-between gap-3 px-3 pb-3">
        <div><p className="font-mono text-[10px] uppercase text-primary">NitinBhaiya Intelligence</p><h1>OI + Premium</h1><p className="text-muted-foreground">ATM ±2 strikes · opening 09:15 ke against COI aur premium behavior.</p></div>
      ...
      <div className="grid gap-3 p-3 lg:grid-cols-[1fr_320px]">
        <Card className="overflow-hidden rounded-none">
          <CardHeader className="py-3"><CardTitle className="flex items-center gap-2 text-sm"><Activity className="h-4 w-4" />Current ATM ±2 Strike Activity</CardTitle></CardHeader>
        ...
        <div className="space-y-3">
          <Card className="rounded-none"><CardHeader className="py-3"><CardTitle className="text-sm">ATM ±2 Net Figures</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-px bg-border p-0">
            {[["CE COI", signed(summary.ceCoi)], ["PE COI", signed(summary.peCoi)], ["CE Premium Δ", signed(summary.cePremiumChange, 2)], ["PE Premium Δ", signed(summary.pePremiumChange, 2)]].map(([label, value]) => <div key={label} className="bg-card p-3"><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className="mt-1 font-mono font-bold">{value}</p></div>)}
          </CardContent></Card>
