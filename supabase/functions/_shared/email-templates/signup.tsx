    ...
    <Body style={body}>
      <Container style={wrapper}>
        ...
        <Container style={card}>
          ...
          <Text style={lead}>
            Thanks for signing up for <strong>{siteName}</strong>. One quick step to activate your account — verify your email address below.
          </Text>
          <Section style={{ textAlign: 'center', margin: '32px 0' }}>
            <Button style={cta} href={confirmationUrl}>Verify my email</Button>
          </Section>
          <Text style={muted}>
          ...
          <Hr style={hr} />
          <Text style={muted}>
            This link was requested for <strong>{recipient}</strong>. If you didn't sign up, you can safely ignore this email — no account will be created.
          </Text>
        </Container>
...
const Footer = ({ siteName, siteUrl }: { siteName: string; siteUrl: string }) => (
  <Section style={footer}>
    <Text style={footerText}>
      © {new Date().getFullYear()} {siteName} · <Link href={siteUrl} style={footerLink}>optionworld.tech</Link>
    </Text>
